//! Custom Draw API (GPU Profiling) Example
//!
//! Demonstrates custom draw with animated uniforms and GPU profiling samples.
//!
//! Notes:
//! - Works on Metal (default macOS) and Blade (`macos-blade` feature).
//! - GPU timestamps are currently available on Metal.
...
use gpui::{
    ...
    CustomDrawParams, CustomDrawResourceStats, CustomFilterMode, CustomFrameDiagnostics,
...
struct GpuProfilingCustomDrawExample {
...
impl GpuProfilingCustomDrawExample {
    ...
    fn update_profile_summary(&mut self, window: &mut Window) {
        match window.take_last_custom_gpu_profile() {
            Ok(Some(profile)) => {
                self.profile_summary = format_profile_summary(profile);
            }
            Ok(None) => {}
            Err(err) => {
                self.profile_summary = format!("GPU profiling unavailable: {err}");
            }
        }
    }
        ...
        let header = div()
            ...
            .child(
                div()
                    ...
                    .child("Custom Draw API (GPU Profiling)"),
            )
            .child(
                div()
                    .text_sm()
...
fn format_profile_summary(profile: CustomGpuFrameProfile) -> String {
    let gpu_time_ms = profile
        .gpu_time_ns
    ...
    format!(
        "draws={}, computes={}, render passes={}, compute passes={}, gpu time={}",
        profile.custom_draw_count,
        profile.custom_compute_count,
...
fn main() {
    application().run(|cx: &mut App| {
        ...
        cx.open_window(
            ...
            |_, cx| cx.new(|cx| GpuProfilingCustomDrawExample::new(cx)),
        )
